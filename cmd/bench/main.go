...
func readReply(conn net.Conn, timeout time.Duration) (string, error) {
...
func runFuncTests(addr string) {
	...
	tests := []testCase{
		...
		{"SET/GET", func(c net.Conn) error {
			...
			r, err := readReply(c, 2*time.Second)
			if err != nil {
				return err
			}
			if r != "+OK\r\n" {
	...
	for i := 0; i < concurrency; i++ {
		...
		go func(workerID int) {
			...
			for j := 0; j < reqsPerWorker; j += pipeline {
				...
				for k := 0; k < batch; k++ {
					t0 := time.Now()
					r, err := readReply(conn, 10*time.Second)
					...
					if err != nil {
						totalErrs.Add(1)
						if err != io.EOF {
							// Connection broken, stop this worker
							return
						}
